...
class GovernmentAgent:
    ...
    def next_action(self):
        ...
        prompt = f"""
                ...
                - 受灾区域待救援人数：{self.env.state[1]}（需要救援的人数）
                - 已被安置群众数量：{self.env.state[2]}（已安置到避难所的人数）
                - 基础设施损坏数量：{self.env.state[3]}（影响资源生产和分配效率）
                - 可用食物资源：{self.env.state[4]}（单位：份，用于满足受灾群众的食物需求）
                - 可用水资源：{self.env.state[5]}（单位：升，用于满足受灾群众的水需求）
                - 可用医疗资源：{self.env.state[6]}（单位：份，用于满足受灾群众的医疗需求）
                - 可用救援装备：{self.env.state[7]}（可用于执行救援和重建任务的装备数量）
                - 可用救援人员数量：{self.env.state[8]}（可用于执行救援和重建任务的人员数量）
                - 可用避难所数量：{self.env.state[9]}（可用于安置受灾群众的避难所数量）
                ...
                - 污染：{self.env.state[14]}（由死亡人数影响，污染过高会影响重建效率）
                - 死亡人数：{self.env.state[15]}（当前已死亡的人数，作为政策制定的参考）
