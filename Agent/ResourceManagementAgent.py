...
class ResourceManagementAgent:
    ...
    def __init__(self, env, llm):
        ...
        self.actions_dict = {
            ...
            1: "安置受灾群众（每安置一个单位消耗可用安置位1单位）",
            2: "基础资源采集（低效率获取食物/水，让已安置群众采集，每安排一个单位的采集消耗1个已安置群众）",
        }
    ...
    def next_action(self):
        ...
        - **未安置受灾群众数量**：{self.env.state[1]}
        - **已安置群众人数**：{self.env.state[2]}
        - **天气条件**：{self.env.state[13]}（0-100，越高越有利）
        - **避难所可容纳人数量**：{self.env.state[9]}

        ### 你的任务是选择最佳的资源分配行动：
        0. **额外发放食物和水**（注意，不进行此食物发放群众也会获得一定量的食物与水，每发放一个单位消耗食物1单位、水1单位，减少群众食物和水需求）。
        1. **安置受灾群众**（每安置一个单位消耗一个安置名额，提升群众安全性和恢复效率）。
        2. **基础资源采集**（由已安置群众进行，低效获取食物或水）。

        ### 你需要考虑：
        - 是否要进行安置操作，安置的人数是否与避难所可容纳人数相当，如果避难所可容纳人数不足，则不应该安置过多的人。
        - 就算不进行食物发放操作，也会给人群提供少量水和食物资源。
