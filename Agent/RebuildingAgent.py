...
class RebuildingAgent:
    ...
    def __init__(self, env, llm):
        ...
        self.actions_dict = {
            0: '建造避难所（每建造一个单位消耗食物10单位、水5单位、医疗资源1单位，建设成员和装备按数量消耗，概率决定建造成功数量，增加避难所容量）',
    ...
    def next_action(self):
        ...
        prompt = f'''
                ...
                - **基础设施损坏数量**：{self.env.state[3]}
                - **可用避难所数量**：{self.env.state[9]}
                - **可用食物资源**：{self.env.state[4]}
                - **可用水资源**：{self.env.state[5]}
                - **可用医疗资源**：{self.env.state[6]}
                - **天气条件**：{self.env.state[13]}（影响重建效率）

                你的任务是选择最佳的重建行动：
                0. **建造避难所**（每建造一个单位消耗食物10单位、水5单位、医疗资源1单位，使用成员和装备按数量消耗，成功率与天气条件相关，增加避难所容量）。
                1. **修复基础设施**（每修复一个单位消耗救援人员1单位、救援装备1单位，成功率与天气条件相关，减少基础设施损坏）。
                2. **污染治理**（每治理一个单位消耗工作成员1单位、装备1单位，成功率与天气条件相关，减少污染）。

                你需要考虑：
                - **当前资源是否足够支持重建行动**，避免不必要的资源浪费。
                - **天气条件是否适合进行重建**，比如天气适应能力（`self.env.state[13]`）对重建效率的影响。
