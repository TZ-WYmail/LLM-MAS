...
class RescueAgent:
    ...
    def next_action(self):
        ...
        prompt = f"""
                ...
                ### 你的任务是选择最佳的救援行动：
                0. **执行搜救行动**（消耗救援队伍，根据天气和灾害强度决定成功率，搜救受灾群众并安置）。
                1. **生产救援队伍**（消耗食物和水资源，增加可用救援队伍数量）。
                2. **医疗救助行动**（消耗医疗资源，优先救治伤员，减少医疗需求）。
                3. **组织撤离**（消耗救援队伍，减少受灾群众数量，降低死亡风险）。
                4. **搭建临时医疗点**（消耗医疗资源和基础设施，提高医疗救治效率）。
        ...
        try:
            ...
            print(f"JSON 解析错误：{e}")
            resource_allocation = self.default_rescue_action
