...
class RescueAgent:
    ...
    def __init__(self, env, llm):
        ...
        self.default_rescue_action = {
            ...
            "reason": "默认行动：执行搜救任务以减少待救援人数，提升生存机会。"
        }
        self.use_memory = True
        self.actions_dict = {
            0: "派出救援队搜救未被救出人员（每出动一人消耗1单位救援人员和1单位装备，成功率受天气影响）",
    ...
    def next_action(self):
        ...
        prompt = f"""
                ...
                ### 你需要考虑：
                - **救援队伍的数量** 是否足够执行搜救等任务，避免资源不足。
                - **天气条件** 是否适合搜救行动，避免高风险任务失败，尤其在恶劣天气下。
                - **食物和水资源** 是否足够支持救援任务和生产新的救援队伍。
                - **医疗资源** 是否充足，是否应优先用于救治伤员。
                - **避难所容量** 是否足够安置新搜救到的受灾群众，避免过多群众无法安置。
